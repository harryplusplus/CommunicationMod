...
```
- CommunicationMod then waits for a message back from the external process, containing a command to be executed.
- Commands may be prefixed by one or more leading options of the form `--key=value`. These options must appear before the command name and may not contain whitespace. Currently supported options are:
  - `--command-id=<value>`
    - Echoes the provided value back in the next JSON response as the top-level field `command_id`.
    - Useful for correlating commands with later state or error messages.
...
- If `--command-id=<value>` was provided, the returned JSON will include `"command_id":"<value>"` at the top level.
